...
\begin{frame}
  ...
  \begin{definition}
    A \emph{knot shadow} is a generic embedding of the unoriented $S^1$
...
\begin{frame}
  ...
  \item<3-> Generate multiquadrangulations by connect sum and take
    dual graphs
  \end{enumerate}
  \begin{center}
    \begin{tabular}{ccccc}
      \onslide<2->{$\vcenter{\hbox{\includegraphics[width=1in]{pre_shadow_graph.pdf}}}$}
      & \onslide<2->{$\Rightarrow$}
      & $\vcenter{\hbox{\includegraphics[width=1in]{result_shadow.pdf}}}$
      & \onslide<3->{$\Leftarrow$}
      & \onslide<3->{$\vcenter{\hbox{\includegraphics[width=1in]{pre_shadow_quads.pdf}}}$}
    \end{tabular}
  \end{center}
...
\begin{frame}
  ...
  \begin{block}{Question}
    ...
    sampling? (c.f.\ Hua, Nguyen, et al. 2005)
  \end{block}
  \begin{figure}
    \centering
    \begin{tabular}{ccc}
...
\begin{frame}
  \frametitle{Questions to answer}
  \begin{block}{Question}
    Given $n_1, n_2$ so that $\PP(\text{an $n_1$-crossing diagram is
...
\begin{frame}
  \frametitle{Questions to answer}
  \begin{block}{Fact}
    No one will realistically enumerate the 100-crossing knot diagrams.
  \end{block}
  \begin{block}{Question}
