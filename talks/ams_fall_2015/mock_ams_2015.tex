...
\title{Asymptotics of Knot and Link Diagrams}
\author[Chapman]{Harrison Chapman}
\date[Mock AMS]{Mock AMS Conference, July 29, 2015}
...
\begin{frame}
  \frametitle{Substructure in diagrams}
  Asymmetry in diagrams results from their tendencies to contain
  specific substructures called tangles
  \begin{definition}
    A \emph{$2k$-tangle} is a diagram-like object having $2k$
    half-edges which lie in the exterior face.

    A tangle is contained in a diagram $D$ if there exists some disk
    which, when intersected with $D$, produces the tangle.
  \end{definition}
  \begin{figure}[h!]
    \centering
    \includegraphics[height=1.5in]{6-tangle.png}
  \end{figure}
\end{frame}
...
\begin{frame}
  \frametitle{Specific tangles appear often}
  In fact, many kinds of tangles appear often almost surely in
  diagrams

  \begin{theorem}[C.]
    Consider a fixed class of diagrams $\KnotDia$ which grows
    ``smoothly'' and let $P$ be a tangle which is appropriately
    ``admissible'' for the class. Then there exist constants $c > 0$
    and $d < 1$ so that
    \[ \Prb(\text{a diagram $D$ in $\KnotDia_n$ contains $\le cn$
      copies of $P$ as a subtangle}) < d^n.\]
  \end{theorem}

  \begin{proof}[Key requirement for proof]
    There is an ``expansion'' operation on diagrams which produces a
    new diagram containing $P$ so that a diagram in $n$ crossings has
    $n/k$ valid expansion sites ($k$ depends on expansion).
  \end{proof}
\end{frame}
...
\begin{frame}
  ...
  \begin{figure}[h!]
    \centering
    \includegraphics[height=1.3in]{fig8_triv_tangle.png}
    \includegraphics[height=1.3in]{silly_2tangle.png}
    \includegraphics[height=1.3in]{fig8_subd_tangle.png}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{figure}[h!]
    \centering
    \includegraphics[height=1.3in]{6-tangle.png}
    \includegraphics[height=1.3in]{6tangle_as_2tangle.png}
  \end{figure}
\end{frame}
...
\begin{frame}
  \frametitle{Smooth growth}
  \begin{caveat}
    The prior theorem has a hypothesis that the class of diagrams
    $\KnotDia$ grow \emph{smoothly}; that
    \[ \lim_{n \to \infty}{|\KnotDia_n|^{1/n}} = \limsup_{n \to
      \infty}{|\KnotDia_n|^{1/n}}.\]
  \end{caveat}

  \begin{itemize}
  \item  The class of rooted link diagrams and the class of rooted prime link
    diagrams are counted exactly and hence known to grow smoothly
  \item Explicit formulas for counts of different classes are still unknown
  \end{itemize}
...
\begin{frame}
  ...
  \begin{theorem}[C.]
    The following additional classes of rooted diagrams grow smoothly;
    \begin{enumerate}
    \item Rooted knot diagrams
    \item Rooted reduced knot diagrams and rooted reduced link
      diagrams
    \item Rooted prime knot diagrams
    \end{enumerate}
  \end{theorem}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item The injection maps are the single and double twists of the root edge
    \vspace{-.7cm}
    \begin{figure}[h!]  \centering
      \[
      \begin{aligned}
        \begin{tikzpicture}[global scale=.7,every path/.style={very thick},decoration={markings, mark=at position 0.5 with {\arrow{>}}}]
      ...
      \begin{aligned}
        \begin{tikzpicture}[global scale=.7,every path/.style={very thick},decoration={markings, mark=at position 0.5 with
        {\arrow{>}}}]
          \node[draw,circle,minimum size=15pt] (A) at (0,0) {$K_1$};
          \node[draw,circle,minimum size=15pt] (B) at (1,0) {$K_2$};
          \node[draw,circle,minimum size=15pt] (C) at (2.5,0) {$K_\ell$};
    ...
    \end{figure}
    \vspace{-.7cm}
  \item Composition is end-to-end composition
       \begin{figure}[h!]  \centering
      \[
      \begin{aligned}
        \begin{tikzpicture}[global scale=.7,every path/.style={very thick}]
