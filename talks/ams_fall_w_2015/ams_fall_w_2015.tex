...
\title{Asymptotics of Random Knot Diagrams}
\author[Chapman]{Harrison Chapman\\University of Georgia\\(some joint w/ Jason Cantarella and Matt Mastin)}
\date[UofM]{Special Session on Algebraic and Combinatorial Structures in Knot Theory\\AMS Western Fall 2015 Sectional\\CSU Fullerton, October 25, 2015}
...
\begin{frame}
  ...
  \begin{theorem}[C.]
    Let $\KnotDia_n$ be the set of rooted knot diagrams with $n$
      crossings.

      Let $P$ be a tangle which is appropriately ``admissible.''

      Then
      there exist constants $c > 0$ and $d < 1$ so that
      \[ \Prb(\text{$D$ in $\KnotDia_n$ contains $\le cn$ copies of
        $P$ as a subtangle}) < d^n.\]
  \end{theorem}
  \pause
  \begin{proof}[Key requirement for proof]
    There is an ``attachment'' operation on diagrams which produces a
    new diagram containing $P$ for which any diagram has $n/k$ valid
    sites ($k$ arbitrary)
  \end{proof}
\end{frame}
...
\begin{frame}
  ...
  \begin{proof}[(Very!) Rough idea of proof]
    ...
    %   \end{figure}
    %   \vspace{-.7cm}
    Connect sum produces diagrams of size $n+m$ from diagrams of size $n$ and $m$:
...
\begin{frame}
  \frametitle{A beautiful bijection}
  Rooted 4-valent maps are in bijection with blossom trees (easy to sample)
  \begin{center}
    \includegraphics[width=4in]{blossom_tree.png}
  \end{center}
  (Figure from Schaeffer, Zinn-Justin 2004)
\end{frame}

\begin{frame}
  \frametitle{Plan for random uniform sampling}
    \begin{fact}
    \begin{itemize}[<+->]
...
\begin{frame}
  \frametitle{SAPs vs. Random Diagrams}
  Can hope to prove \emph{other} theorems which are true for SAPs. \pause

  For example, adapting a proof of Madras;
  \begin{theorem}[C.]
    Let $k_n^*$ be the count of diagrams representing the most common
    knot among all knots in $\KnotDia_n$. Then,
    \[ \lim_{n\to\infty}{(k_n^*)^{1/n}} = \lim_{n\to\infty}{|\KnotDia_n|^{1/n}}. \]
  \end{theorem}\pause
  Namely, the FWD theorem for diagrams shows that
  \[ \lim_{n\to\infty}{(k_n^*)^{1/n}} >
  \lim_{n\to\infty}{|\{\text{$n$-crossing $0_1$
      diagrams}\}|^{1/n}}, \] so there is some $n$ and some
  non-trivial knot type which is more common than the unknot in
  $\KnotDia_n$.
\end{frame}
