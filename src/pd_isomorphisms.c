...
/* Given a component permutation, we can build the set of edgemaps
   ...
   to ALL components of pdB by iterating over a multi-index composed
   of CYCLIC groups. */
...
pd_iso_t **pdint_build_map_isomorphisms(pd_code_t *pdA,pd_code_t *pdB,
                                        unsigned int *nisos,bool FAILEARLY)
{
  /* For the pdcodes to even have a chance of being
     isomorphic, the hash codes, # crossings, # edges, #
...
pd_iso_t **pd_build_map_isomorphisms(pd_code_t *pdA, pd_code_t *pdB, unsigned int *nisos) {
    return pdint_build_map_isomorphisms(pdA, pdB, nisos, false);
}

bool pd_map_isomorphic(pd_code_t *pdA,pd_code_t *pdB)

/* This is basically just a wrapper for pd_build_map_isomorphisms. */

{
    pd_iso_t **iso_buf;
    unsigned int nisos;

    iso_buf = pdint_build_map_isomorphisms(pdA,pdB,&nisos,false);
