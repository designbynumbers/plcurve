/*
  ...
  This file generates closed and confined random walks using toric-symplectic Markov chain Monte Carlo (TSMCMC)
...
struct arg_int *burn_in;       // number of steps to discard in burn-in period (optional)
struct arg_file *outfile;      // name for outfile (optional)
struct arg_str *format;       // Vect, Mathematica, ChordLength (optional)
struct arg_str *knot_type;     // knot type of samples to look for
struct arg_dbl *ftc;          // Failure to close
struct arg_int *chord_skip;    // If the output format is ChordLength, which chord length to record
...
int main(int argc, char *argv[]) {
    ...
    void *argtable[] =
            {
                    ...
                    knot_type = arg_str0("k", "knot-type", "<3_1>", "knot type of samples to generate"),

                    radius = arg_dbl0("r", "radius", "<float>", "radius of confinement sphere (optional)"),
                    ftc = arg_dbl0(NULL, "failure-to-close", "<float>",
                                   "distance between first and last vertex (optional, don't use for closed polygons"),
                    skip = arg_int0(NULL, "skip", "<int>", "number of walks to skip between samples"),
                    burn_in = arg_int0("b", "burn_in", "<int < nsamples>", "number of samples to discard"),
                    mpr = arg_int0(NULL, "moment-polytope-repeat", "<10>",
                                   "number of times to repeat moment polytope step (optional)"),
                    seed = arg_int0(NULL, "seed", "<int>", "seed for random number generator (optional)"),
                    beta = arg_dbl0(NULL, "beta", "<float> in [0,1]",
                                    "fraction of non-permutation steps which are moment polytope steps"),
                    delta = arg_dbl0(NULL, "delta", "<float> in [0,1]",
                                     "fraction of all steps which are permutation steps"),
                    ...
                    chord_skip = arg_int0(NULL, "chord_skip", "<int> in [0,n]",
                                          "number of edges to skip when measuring chord length"),
    ...
    if (ftc->count > 0) {
        ...
        runftc = ftc->dval[0];
        if (runftc <= 0.0 || runftc >= (double) (runn)) {
            fprintf(stderr, "randompolygon: failure-to-close must be in (0,n) for any polygons to exist");
            exit(1);
        }
    ...
    if (burn_in->count > 0) {
        ...
        runburnin = burn_in->ival[0];
        if (runburnin < 0) {
            fprintf(stderr, "randompolygon: number of samples to discard during burn_in = %d must be > 0", runburnin);
            exit(1);
        }
    ...
    if (skip->count > 0) {
        ...
        runskip = skip->ival[0];
        if (runskip < 0) {
            fprintf(stderr, "randompolygon: number of walks to skip between samples = %d must be >= 0", runskip);
            exit(1);
        }
    ...
    if (beta->count > 0) {
        ...
        runbeta = beta->dval[0];
        if (runbeta < 0 || runbeta > 1.0) {
            fprintf(stderr, "randompolygon: beta = %g must be in [0,1].", runbeta);
            exit(1);
        }
    ...
    if (delta->count > 0) {
        ...
        if (rundelta < 0 || rundelta > 1.0) {
            fprintf(stderr, "randompolygon: delta = %g must be in [0,1].", rundelta);
            exit(1);
        }
    ...
    if (mpr->count > 0) {
        ...
        runmpr = mpr->ival[0];
        if (runmpr < 0) {
            fprintf(stderr, "randompolygon: number of times to repeat moment polytope steps = %d must be > 0", runmpr);
            exit(1);
        }
    ...
    if (runof == ChordLength) {
        ...
        if (chord_skip->count > 0) {

            chordskipk = chord_skip->ival[0];
    ...
    if (knot_type->count > 0) {
        searchtype = plc_read_knottype(knot_type->sval[0]);
        if (searchtype == NULL) {
    ...
    if (verbose->count > 0) {
        ...
        if (radius->count > 0 || ftc->count > 0) {
            ...
            printf("Algorithm Parameters\n"
                   ...
                   "# of walks to discard during burn_in period : %04d\n"
    ...
    if (runof == Mathematica || runof == VECT) {
        ...
        if (sampledir != NULL) { /* Directory exists; kill it */
            ...
            if (remove(outfile_name) != 0) {
                ...
                fprintf(stderr, "randompolygon: Couldn't remove existing directory ./%s to make room for samples",
                        outfile_name);
                exit(1);
            ...
            if (searchtype != NULL) {
                ...
                if (thistype == NULL) { /* We couldn't classify the knot: fail */
                    ...
                    for (check = 0; check < nposs; check++) {
                        ...
                        if (thistype->cr[check] == searchtype->cr[check] &&
                            thistype->ind[check] == searchtype->ind[check]) {
    ...
    if (diagonal_lengths != NULL) {
        free(diagonal_lengths);
        diagonal_lengths = NULL;
    }
    if (dihedral_angles != NULL) {
