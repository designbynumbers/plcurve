                    ...
                    for pd, xmask in PDStoreExpander.crossing_combinations(comp_pd):
                        ...
                        if not (sign_count in iso_classes and pd in iso_classes[sign_count]):
                            ...
                            if (homfly in self.ambiguous_homflys and
                                ...
                                if args.showTypes:
                                    print str(kt)
                                if args.knottype == str(kt):
                                    print "Found " + args.knottype
                                    pd.write(outfile)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Extract knots of a given type in a pdstor file, trying all orientation/crossing sign variations of all diagrams in the pdstor file')
    parser.add_argument(
        '--knottype',
        help="knot type to search for, e.g. Knot[3, 1]")
    parser.add_argument(
        '--pdstor', type=argparse.FileType('r'),
        help='.pdstor file to search')
    parser.add_argument(
        '--showTypes',help="display all knot types found",action="store_true")

    args = parser.parse_args()

    dashify = re.compile('\[| ')
    delchar = re.compile('\]|\,')
    outfilename = dashify.sub('-',args.knottype.lower())
    outfilename = delchar.sub('',outfilename) + ".pdstor"

    print "*********************************************************"
    print "pdextract 1.0: Searching for " + args.knottype
    print "placing results in " + outfilename
    print "*********************************************************"
