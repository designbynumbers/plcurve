...
octrope_link, octrope_linkstruct, octrope_link_new,
octrope_link_free, octrope_link_read, octrope_link_write,
octrope_link_edges, octrope_link_fix_wrap, pline - interface
functions and data types for storing links in the B<octrope>
library.

=head1 LIBRARY

B<Octrope> library (liboctrope, -loctrope)
...
typedef struct octrope_pline_type {
  int             acyclic;   /* True if pline has distinct ends */
...
I<octrope_vector>, the I<octrope_pline>, and the I<octrope_link>. 

As expected, the I<octrope_vector> is a 3-vector.

=head2 The I<octrope_pline> type

The I<octrope_pline> is a bit more subtle. The data type contains a buffer 
of I<nv> vertices and the I<acyclic> tag. The value of I<acyclic> is 
TRUE (nonzero) when the space polygon has two ends, and FALSE (zero) when the
space polygon is closed. For example, a line segment has I<nv> == 2 and 
I<acyclic> == TRUE, while a triangle has I<nv> == 3 and I<acyclic> == FALSE.
...
when I<acyclic> == FALSE. As long as the extra memory is correctly
...
The I<octrope_pline> type also contains space for a buffer of colors,
...
I<octrope_pline>.  No facility has been provided in this version of 
B<Octrope> to work with these plines separately, and we discourage
taking I<octrope_pline> types out of their I<octrope_link>
...
I<components> I<octrope_pline> structures where the ith I<octrope_pline>
...
buffers I<nv> and I<acyclic> are copied into new memory locations inside
I<octrope_link_new>. It is hence safe to free them after a call to 
I<octrope_link_new> if desired.

The I<acyclic> array contains TRUE in I<acyclic[i]> if the ith component
...
above, we note that the vertex buffers in the I<octrope_pline> members
