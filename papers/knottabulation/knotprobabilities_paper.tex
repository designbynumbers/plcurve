...
\newcommand{\hopfgraph}{\raisebox{-0.17\baselineskip}{\includegraphics[height=0.81\baselineskip]{hopfgraph}}}
...
\Comment{Build all isomorphisms (if any) between pdcodes $P$ and $P'$}
...
\ForAll{component-length preserving $\gamma: \{c_1,\dots, c_C\} \rightarrow \{c'_1, \dots, c'_C\}$}
	\Comment{There is at least one such $\gamma$ because the hashes match}
	\ForAll{compatible and component-preserving $\epsilon$}
	\Comment{Generated by iterating over the product of dihedral groups $D_{\edges(c_1)} \times \cdots \times D_{\edges{c_C}}$.}
	\ForAll{collections of orientations of components}
	\If{a compatible and consistently oriented $\nu$ exists}
	\Comment{ince each vertex of $P$ is incident to four edges in $P$, and the corresponding edges in $P'$ might not even share a vertex of $P'$ in common}
			\If{the edges $e_i, e_j, e_k, e_l$ around \emph{each} vertex $v$ in $P$ (in counterclockwise order) map under $\epsilon$ to the edges around $\nu(v)$ (in counterclockwise order)}
...
If the edge multiplicity is four, $G$ is \hopfgraph. This is obtained from the graph with one edge and two vertices by two $\edgedouble$ moves. 
