...
\begin{document}
\title[]{An Enumeration of Knot Diagrams}
...
There is currently a great deal of interest in the topic of random geometry and random topology. The study of random knots goes back to the 1960's, when polymer physicists realized that the knot type of a closed (or ring) polymer would play an important role in the statistical mechanics of the polymer~\cite{Edwards:1967hd}. Orlandini and Whittington give a comprehensive survey of the development of the field since~\cite{Orlandini:2007kn}. In this area, a random knot is usually modeled by a closed random walk, often on the integer lattice. For almost all of these models, certain phenomena have been rigorously established-- for instance, the probability of knotting goes to 1 exponentially fast as the size of the walk increases. However, it has been difficult to prove more informative theorems. 

One way to look at the problem is that it is quite hard to relate the three-dimensional shape of a space curve to its knot type; one can in principle express the finite-type invariants as (very complicated) integrals in the spirit of \cite{Lin:1994wq}, but so far computing the expected value of these integrals has been too complicated to pull off. In experimental studies, one generates random curves, projects them to knot diagrams, and computes invariants from the diagrams. The point of this paper is to look at the set of knot diagrams not as convenient combinatorial representations of the shape of space curves, but as a probability space in their own right. 

The objects in our space will be equivalence classes of immersions of $S^1$ into $S^2$ as in Arnol'd~\cite{Arnold:1994wr}. We call this the set of knot shadows (or in Kauffman's language, knot universes~\cite{knots and physics}). We then write the $n$-crossing knot diagrams as the product of the $n$-crossing knot shadows and the hypercube ${\pm 1}^{n+1}$ representing assignment of global orientation and crossing signs. We carried out an enumeration of composite knot diagrams up to 11 crossings and prime knot diagrams up to 15 crossings using the graph theory software~\emph{plantri}~\cite{Brinkmann:2005um,Brinkmann:2007up}.

Looking at this data, we are led to several conjectures:

\begin{conjecture} 
The fraction of knot shadows with nontrivial automorphism group approaches 0 exponentially fast as $n \rightarrow \infty$.
\end{conjecture}

\begin{conjecture}
Most diagrams are highly composite. The mean number of prime factors of a diagram $\rightarrow \infty$ as $n \rightarrow \infty$ and the number of prime factors of a random diagram is strongly concentrated around this expectation.
\end{conjecture}




The analysis of closed random walks plays an important role in the physics of ring polymers. The configuration space of a ring polymer is divided into chambers corresponding to different knot types, and understanding the relative sizes and shapes of these chambers determines the statistics of the theory~,


 Certain features of these models, such as the limiting ratio of probabilities of different knot types~\cite{JANSEVANRENSBURG:2011jo}, or the fact that the probability of the unknot goes to zero, appear to be universal features of these models. Why?

In practice, knot types are always computed in these studies by constructing a planar knot diagram and computing topological invariants from the combinatorics of the diagrams. This leads to the following idea: each model implicitly pushes forward a probability distribution on a space of curves to a probability measure on a space of diagrams and then to a probability measure on the space of knot types. Perhaps the pushforward distributions on knot space share common features becuause 

Our overall goal is to introduce a middle ground between topology and geometry; a model of random knots as random combinatorial objects which is close enough to topology to prove theorems using the tools of classical knot theory and at the same time close enough to geometry to have a hope of understanding the induced distribution on random combinatorial objects generated by a given model of random curves. With these constraints in mind, the natural combinatorial object to consider is the knot diagram, which has well-developed links to both curves and invariants. So this point of this paper is to initiate a study of random knot diagrams. 

Evan-Zohar, Hass, Linial, and Nowik~\cite{EvenZohar:2014ws} solve this problem by introducing a combinatorial model of knotting which converts permutations into knots directly. This allows them to prove powerful new theorems.

We proceed as follows. There are a finite number of topologically distinct\footnote{We will make the equivalence relation on diagrams precise below.} immersions of circle into the sphere with a fixed number of transverse double points $n$. We tabulate these by computer. Assigning over/under information to the double points creates the set of knot diagrams with $n$ crossings. We then classify the knot type of each diagram using standard invariants. This allows us to provide exact answers to questions such as these: What fraction of 9 crossing knot diagrams are unknots? <FILL IN THE NUMBER HERE> How many crossings can be removed by performing only Reidemeister I moves on an average 10 crossing diagram? <FILL IN THE NUMBER HERE> What is the second moment of the $c_2$ invariant on random 11 crossing diagrams? <FILL IN THE NUMBER HERE> This is basically an experimental paper, though we devote some mathematical effort to formally proving that our algorithm for diagram tabulation is correct in Sections~<FILL IN SECTIONS>. 
...
A more recent related work is that of , who introduced a fascinating random knot model where the underlying combinatorial object is a random permutation, using Adams' idea of a Petal diagram~(cite adams paper here). This model was designed to be analyzed, and they proved some very striking and beautiful results on the distribution of certain knot invariants in this model. If these are universal properties of all random knot models coming from combinatorics, they will prove very useful in polymer physics. However, that specific model is unlikely to solve our particular problems soon: it is hard to see much hope of connecting random curves to random Petal diagrams in way that's explicit enough to know much about the induced distribution on Petal diagrams. 

There is also a long history of deep combinatorial results about counting knot diagrams, usually as a way of building knot tables or counting knot types. Conway's famous paper on knot tabulation~\cite{Conway:1970vr} gives a strategy for enumerating diagrams based on an enumeration of equivalence classes of rational tangles and the Tutte's enumeration of a class of planar ``c-nets''~\cite{Tutte:1963gz} which Conway used as a kind of skeleton for the diagrams. Thistlethwaite and Sundberg~\cite{Sundberg:1998hr} studied the asymptotics of this model to prove results about the number of alternating links. These papers don't directly count or enumerate diagrams in the same sense that we do (the tangle counts are up to additional equivalence relations coming from knot theory), but they could certainly be modified to address our case. 

Similarly, Arnol'd's seminar studied the immersions of the circle into the plane in the 1990's as part of his general theory of finite type invariants of plane curves. Arnol'd's main paper actually provides an explicit count of the same immersions of $S^1$ into $S^2$ that we enumerate, but only for $n \leq 5$~\cite[page 79]{Arnold:1994wr}. This is sequence A008989 in the Online Encyclopedia of Integer Sequences~\cite{Untitled:vPCfHsyG}, with an extension to $n=7$ credited to Guy H.\ Valette. We could not find a published reference for Vallette's extension of the table, but our data for knots below does match A008989 including the extension.
...
\begin{figure}
	...
	\end{overpic}
	\caption{\label{fig:ShadowExamples}Examples of links shadows.}
\end{figure}
...
Brinkmann et.\ al.~\cite{Brinkmann:2007up} provided an algorithm to enumerate all of the simple embedded planar quadrangulations of $S^2$, up to embedded isomorphism. A quadrangulation is a planar graph where each face has four edges, as in Figure \ref{fig:QuadExamples}.
...
\hfill
\hphantom{.}
\caption{\label{fig:NonSimpleQuad} An example of a non-simple quadrangulation (right) and its dual graph (left).}
\end{figure}
...
\begin{center}
  \begin{tikzpicture}[->,,shorten >=1pt,semithick,align=center]
    ...
    \node[left=1in of prime2]  (prime1) 
    {Prime diagrams\\\tt{(plantri)}};
    \node[right=1in of prime2] (all)    
    {All diagrams\\of $n$ crossings};

    \node[below=.7cm of prime2] (sum2) {2-summand\\diagrams};
    \node[below=.7cm of sum2]   (sum3) {3-summand\\diagrams};
    \node[below=1.4cm of sum3]    (sumn) {$n$-summand\\diagrams};
...
\noindent where the arrows indicate ``all possible connect sums (and entry into the database with isomorphic copies of an existing diagram rejected)'' or simply ``entry into the database with isomorphic copies rejected''.
...
\caption{\label{fig:MultThree} By parity, because we came from a 4-regular embedded planar graph only the leftmost case can occur at any stage in the reduction process.}
\end{figure}
