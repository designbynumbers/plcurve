...
We also observe that two pairs of vertices $ab$ and $cd$ on the unit circle may be joined by nonintersecting chords inside the circle if and only if the pairs are unlinked on the circle (\raisebox{-0.032in}{\includegraphics[width=0.15in]{unlinked-on-circle.pdf}} instead of \raisebox{-0.032in}{\includegraphics[width=0.15in]{linked-on-circle.pdf}}). This happens when the order of the vertices around the circle is $abcd$ instead of $acbd$ or $adbc$. 
