...
\noaffiliation
\author{Matt Mastin}
\altaffiliation{MailChimp, Atlanta, GA}
\noaffiliation
...
Since number of vertices and edges of $\mathfrak{L} \#_{e,e'} \mathfrak{L'}$ is the sum of the vertices and edges in $\mathfrak{L}$ and $\mathfrak{L'}$ and the number of faces is 2 less than the sum of the number of faces in $\mathfrak{L}$ and $\mathfrak{L'}$, $V - E + F = (2-2g) + (2-2g') - 2 = 2-2(g+g')$ for $\mathfrak{L} \#_{e,e'} \mathfrak{L'}$, so the connect sum pd-code is of genus $(g+g')$ as desired.
\end{proof}
