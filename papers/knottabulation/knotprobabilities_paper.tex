...
The statistical mechanics of closed (or ring) curves have long been of interest in polymer science (maybe cite Treloar or something?). The basic model is that one assumes that the shape of a polymer in solution is chosen randomly from some probability distribution on closed curves of a certain length, and studies the statistical properties of those random curves in order to understand the average properties of the polymer. Since the 1960's, physicists have realized~\cite{Edwards:1967hd} that knot type has to play an important role in this theory; a given ring polymer cannot sample the entire space of closed curves, but is restricted to the connected component of curves with the same knot type. This insight gives rise to a number of natural questions: if we partition the space of closed curves into knot types, what are the relative sizes of the various components? How much of the space consists of unknots? What happens to the fraction of unknots as the curves become longer? What is the average topological complexity of a random curve of given length?

It is now the case that there are quite satisfying mathematical answers to some of these questions, while many others still stubbornly resist rigorous analysis. The survey paper of Orlandini and Whittington \cite{Orlandini:2007kn} gives six models for random curves; for example, we could model closed curves by self-avoiding lattice walks, or by random closed equilateral polygons. In each model, it's known that the fraction of unknots approaches zero rapidly as the size of the curve increases. However, while there are a wealth of experimental studies, it has been very difficult to prove more theorems about random knotting. One way to look at the problem is that the existing tools of knot theory are based on knot diagrams rather than knot shapes, and it is difficult to extract topological information from a probability distribution defined directly in terms of geometry.

Our overall goal is to try to improve this situation by finding a middle ground between topology and geometry; a model of random knots as random combinatorial objects which is close enough to topology to prove theorems using the tools of classical knot theory and at the same time close enough to geometry to have a hope of understanding the induced distribution on random combinatorial objects generated by a given model of random curves. With these constraints in mind, the natural combinatorial object to consider is the knot diagram, which has well-developed links to both curves and invariants. So this point of this paper is to initiate a study of random knot diagrams. 
