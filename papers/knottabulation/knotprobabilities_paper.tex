...
One way to look at the problem is that it is quite hard to relate the three-dimensional shape of a space curve to its knot type; one can in principle express the finite-type invariants as (complicated) integrals in the spirit of \cite{Lin:1994wq}, but so far computing the expected value of these integrals has been too difficult. In experimental studies, one generates random curves, projects them to knot diagrams, and computes invariants from the diagrams. The point of this paper is to look at the set of knot diagrams not as convenient combinatorial representations of the shape of space curves, but as a probability space in their own right. 

The objects in our space will be equivalence classes of immersions of $S^1$ into $S^2$ as in Arnol'd~\cite{Arnold:1994wr}. We call this the set of knot shadows (or in Kauffman's language, knot universes~\cite{knots and physics}). We then write the $n$-crossing knot diagrams as the product of the $n$-crossing knot shadows and the hypercube $\{\pm 1\}^{n+1}$ representing assignment of global orientation and crossing signs. We carried out an enumeration of composite knot diagrams up to 11 crossings using the graph theory software~\emph{plantri}~\cite{Brinkmann:2005um,Brinkmann:2007up}. We checked our results by using~\emph{plantri} in two different ways to generate our graphs, as described below.

How many of these diagrams represent nontrivial knots? As in most models of random knotting, the fraction of unknots is very high for small crossing numbers and begins to decay as the number of crossings in the diagram increases. Interestingly, in this range, the unknot fraction is relatively easy to understand. For some diagrams (called ``tree-like'' curves), the knot type does not depend on the assignment of crossing information-- the resulting knot is \emph{always} the unknot (see Figure~\ref{fig:treelike}). These diagrams turn out be surprisingly common for low crossing numbers. For example, $42.05\%$ of 8-crossing diagrams are treelike, which explains about half of the unknots among 8-crossing diagrams ($84\%$). The remaining unknots are almost all connect sums of treelike diagrams with a single trefoil or figure-8 diagram. We will show that a simple analysis based on such diagrams gives a lower bound of $77\%$ for the unknot fraction in 8-crossing diagrams, explaining more than $90\%$ of all unknots in this class. Motivated by this, we then focus on the connect sum structure of random diagrams. This is extremely interesting, as our data shows that the vast majority of diagrams are highly composite. This is partially explained by the existence of pattern theorem for diagrams~\cite{harrison}, but more work remains to characterize the distribution of various sizes of summands in a random diagram. Counting knot diagrams is made significantly more complicated by diagram symmetries. Our data shows that symmetric diagrams are very rare among diagrams; the average size of the automorphism group for a random 10 crossing diagram, for instance, is $846929/823832 \simeq 1.028$. 
...
\end{figure}
It is well-understood that the equivalence relation preserves the (spherical) embedding of the planar graph; in fact, the left and right-most shadows are actually equivalent under embedded isomorphism-- we have just changed the ``exterior'' face of the projection from the sphere to the plane. We can partition the edges of a link diagram into \emph{components} by defining two edges to be equivalent if they meet at a vertex at positions which are not cyclicly adjacent. We will call shadows with a single component \emph{knot shadows}. They will be the focus of this paper.
...
Our first goal was to enumerate the link shadows-- that is, the connected 4-regular embedded planar (multi)graphs, up to embedded isomorphism-- computationally. This section describes our two enumeration algorithms. Each is built on the same computational foundation; a library which allows us to describe and manipulate diagrams as a combinatorial object called a \emph{pd-code}. In this library, we provide a data structure for shadow storage which rejects shadows which are isomorphic to ones already present in the database. This is done using a combination of hashing\footnote{We hash on a combination of features such as the number of faces with a given number of edges, which must agree in isomorphic diagrams.} and brute-force search. Details are provided in Appendix A for the curious reader. 
...
\end{figure}
The \plantri software of Brinkmann and McKay~\cite{Brinkmann:2007up,McKay:1998wa} can be used to generate the dual graphs of the planar simple quadrangulations of given number of vertices. Since the number of vertices in the dual graph of a quadrangulation is 2 less than the number of vertices in the quadrangulation, we can use this to enumerate the connected 4-regular embedded planar multigraphs which are prime link shadows.

To enumerate all shadows, we recall (as is standard in knot theory) that every composite shadow is uniquely expressed as a collection of prime shadows joined along pairs of edges by the connect sum operation. Thus, our algorithm is simply a way to organize this process of taking connect sums. The results are hard to predict theoretically, because we can generate the same shadow in more than one way when one of its prime summands has a nontrivial automorphism group. The code is simple, however, since we need only come up with a process which must generate each shadow (eventually) and insert all the shadows we generate  into our isomorph-rejecting database.

We can observe that any $n$-crossing shadow is obtained by connect summing prime diagrams whose crossing numbers partition $n$. However, order matters, as some connect sums of 3, 1, and 3 crossing diagrams have to be performed in the order $3 \# (1 \# 3)$ and will not appear if we connect sum in the order $1 \# (3 \# 3)$. A similar  
