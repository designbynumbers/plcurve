...
The previous authors have tabulated knot shadows. To pass from shadows to diagrams, we must assign crossing signs and an orientation to the diagram. In principle, there are as many as $2^{\cr + 1}$ distinct ways to do this, but symmetries of the diagram may reduce this number-- for instance, the three crossing shadow of the trefoil knot $\tref$ has a 3-fold symmetry cyclically permuting the crossings, so the crossing assignments $+--$, $-+-$, and $--+$ are all the same.
...
\end{tabular}
\end{ruledtabular}
\caption{The number of link (including knots) and knot shadows and diagrams through 10 crossings. The unstarred numbers in the column of prime shadows are sequence A113201 in the OEIS. The unstarred numbers in the table of link shadows matches K\'apolnai et.\ al.~\cite{Kapolnai:2012hs}, while the unstarred numbers in the column of knot shadows are sequence A008989 in the OEIS. The starred numbers are new.}
\label{tab:counts}
\end{table}

Looking at Table~\ref{tab:counts}, one is struck by how close the number of distinct knot diagrams is to the maximum number $2^{\Cr+1} \times (\# \text{ knot shadows})$.
\section{Results}
