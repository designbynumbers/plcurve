...
\end{figure}
The dual graph to a quadrangulation is a connected 4-regular embedded planar multigraph, as shown at left in Figure~\ref{fig:NonSimpleQuad}. In other words, the dual graph defines a link shadow. This is almost enough to enumerate shadows. However, not every link shadow is obtained in this way: if the quadrangulation is simple, no pair of faces in the link shadow share more than one edge. As we can see in Figure \ref{fig:NonSimpleQuad}, this property is not true for every link shadow.
\begin{figure}
\hphantom{.}
\hfill
...
\hphantom{.}
\hfill
\includegraphics[height=1.25in]{connect-sum}\hfill
\hphantom{.}
...
\begin{proposition}
Every composite link shadow can be created by connect sum operations on a well-defined set of prime link shadows.
...
\hphantom{.}
\hfill
\includegraphics[height=1.25in]{undo-connect-sum}\hfill
\hphantom{.}
...
\hphantom{.}
\hfill
\includegraphics[height=1.25in]{cuts-commute}\hfill
\hphantom{.}
...
\begin{lemma}
The set of (diagram-isomorphism classes) $\{L_1, \dots, L_k\}$ of the prime summands of $L$ is a shadow-isomorphism invariant of $L$. In particular, the set of numbers of vertices $\{V_1, \dots, V_k\}$ is a shadow isomorphism invariant of $L$.
...
We implemented the above algorithms in C and used them to generate a list of knot shadows up to 10 crossings, each unique up to pd-isomorphism. The longest run took several days on a desktop computer. We then assigned orientations and over-under information at the crossings to each shadow to make a list of diagrams. We eliminated diagram-isomorphic duplicates from this list to arrive at a database of diagrams. We classified the knot type of each diagram as it was generated using the Millett/Ewing HOMFLY-PT code (if possible), and stored diagrams with ambiguous knot types for further analysis using Bar-Natan and Morrison's KnotTheory Mathematica package. This part of the computation was significantly more costly-- the 10 crossing diagrams ran for several days on an average of 200 cores provided by the Amazon EC2 cloud computing environment. The counts of shadows and diagrams are given in Table~\ref{tab:counts}.
...
\begin{proof}
In each case, there is a group action on diagrams (reverse all crossings of a diagram, reverse the orientation of a diagram, or take the 4-element group generated by both operations) which converts diagrams of one knot of these knot types to a diagrams of the other types. This action extends to diagram-isomorphism classes of diagrams because the action commutes with any given diagram-isomorphism. 

It is not always the case that this action is free on diagram-isomorphism classes of diagrams of any knot type. For instance, taking the mirror image of this diagram of the $7_4$ knot \raisebox{-0.4 \height}{\includegraphics[height=0.25in]{74knot.png}} is equivalent to rotating it by 180 degrees (a diagram-isomorphism). But this merely shows that $7_4$ is a reversible knot, as diagram-isomorphisms are knot isotopies. By hypothesis, $K$ is \emph{not} isotopic to its images under the group action, and therefore it cannot by diagram-isomorphic either.

Since the action of the group of order 2 (or 4) on this set of diagram-isomorphism classes is free and exchanges the various knot types, the number of diagram-isomorphism classes of diagrams in each knot type must be the same.
\end{proof}

Using this lemma, we could safely split our original count of $\num{5672}$ 10-crossing diagrams of base type $8_{17}$ into $\num{2836}$ diagrams of type $8_{17}$ and $\num{2836}$ diagrams of type $8_{17}^r$, even though we have no way of knowing which of our diagrams to assign to each of these knot types. 

The relative frequencies of all the knot types appear in Figure~\ref{fig:knot frequency loglog} in a log-log plot. The figure also shows the 10 most frequent knot types, from the unknot (the most frequent knot type in all our data) and the trefoils $3_1$ and $3_1^m$ through the square knot $3_1 \# 3_1^m$ and the $6_3$ knot, which is the 10th most common knot in all our data. Though we know~\cite{chapman} that unknotted diagrams eventually become exponentially rare, and so that the rank-order of the knot types must eventually change, we do not see this effect in our data; the list of 10 most common knots is the same for crossing numbers 6-10. 

More interestingly, the log-log plot of ranked knot frequencies for 10 crossing knots is roughly linear over 9 orders of magnitude (there are about $1.6 \times 10^9$ diagrams, with the most frequent knot type (the unknot) occurring $1.2 \times 10^9$ times and the least frequent knot types (a 98-way tie among various 10-crossing knots) observed exactly once. It is not clear to us why this phenomenon should occur in the data (and it may disappear for larger crossing numbers), but it certainly seems worthy of further investigation!
