...
Our first goal was to enumerate the link shadows computationally. This section describes our two enumeration algorithms. Each is built on the same computational foundation; a library which allows us to manipulate pd-codes. In this library, we provide a data structure for pd-code storage which rejects pd-codes which are pd-isomorphic to ones already present in the database. This is done using a combination of hashing\footnote{We hash on a combination of features such as the number of faces with a given number of edges, which must agree in isomorphic diagrams.} and brute-force comparison. Details are provided in Appendix A for the curious reader. We are left with the problem of creating a set of inputs to this data structure which are guaranteed to cover all $n$-vertex shadows.
...
\end{definition}
This is not the same thing as prime and composite knots-- we can assign crossings and get a prime diagram of a composite knot or a composite diagram of a prime knot, as in Figure~\ref{fig: prime and composite}-- but the theory is quite similar.
...
This much of the theory is simple and pleasant; and it leads to a natural guess for an algorithm-- since $n$ crossing link shadows must be assembled from collections of prime link shadows with numbers of crossings that sum to $n$, we can start by enumerating partitions of $p_1 + \dots p_k = n$ then take all choices of diagrams with $p_1, p_2, \dots, p_k$ crossings and then take all ways to connect sum these diagrams. The main problem here is in computationally specifying ``all ways'' to connect sum the diagrams. It is evident that in a composite diagram, some prime summands are connect summed and others are not. It turns out that the graph of incidences looks like a tree of complete graphs, where the order of the complete graph gives the number of adjacent edge pairs on each pair of faces sharing more than one edge. Building the connect sums this way would be interesting, but intricate and difficult to verify. Further, it would be wasteful, because we'd be unable to reuse common smaller composite diagrams that recurred in different $n$ crossing diagrams.

Instead, we can proceed iteratively without too much waste using some simple observations: We are interested in relatively low $n$, so the partitions of $n$ can be enumerated without difficulty. We can choose where to end the construction, so we might as well add the largest prime diagram last. Symmetries can lead us to construct the same composite diagram by connecting different edges of diagrams.



Instead, 

now we face some difficulties. The most obvious one is that the connect sum a given set of prime link diagrams requires us to specify 

 and particular edges to cut-and-splice at each stage. 

an example of a non-simple quadrangulation and its (composite) dual graph are shown inThe \plantri software of Brinkmann and McKay~\cite{Brinkmann:2007up,McKay:1998wa} can be used to generate the dual graphs of the planar simple quadrangulations of given number of vertices. Since the number of vertices in the dual graph of a quadrangulation is 2 less than the number of vertices in the quadrangulation, we can use this to enumerate the connected 4-regular embedded planar multigraphs which are prime link shadows. The question is then how to go from prime diagrams to composite ones.
