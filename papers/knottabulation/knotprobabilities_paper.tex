...
\newcommand{\twistunknot}{\raisebox{-0.17\baselineskip}{\includegraphics[height=0.81\baselineskip]{twistunknot.pdf}}}
...
The study of random knots goes back to the 1960's, when polymer physicists realized that the knot type of a closed (or ring) polymer would play an important role in the statistical mechanics of the polymer~\cite{Edwards:1967hd}. Orlandini and Whittington~\cite{Orlandini:2007kn} give a comprehensive survey of the development of the field in the subsequent years. Most random knot models are based on closed random walks, and there are many variants corresponding to different types of walks (lattice walks, random walks with fixed edgelengths, random walks with variable edgelengths, random walks with different types of geometric constraints such as fixed turning angles). For almost all of these models, certain phenomena have been rigorously established-- for instance, the probability of knotting goes to 1 exponentially fast as the size of the walk increases\cite{Pippenger:1989ft,Soteros:1992PSP,Diao:1993JKTR,Diao:1995iw,Sumners:1999cd,Diao:2001db}. However, it has been difficult to prove more informative theorems.
...
Figure~\ref{fig:knot frequency loglog} shows the relative frequency of all the
knot types we observed, sorted by rank order among knots. When plotted on a
log-log plot, we see that the plot is roughly linear across 9 orders of
magnitude-- giving some evidence for a roughly Zipfian distribution of knot
types. In such a distribution, the $k$-th most frequent knot type would have a
probability proportional to $k^{-s}$ for some $s$. It would be very interesting
to know what happens for large $n$. This distribution has been previously
observed for knotting in self-avoiding polygon models by Baiesi, et
al.~\cite{Orlandini:2007PRL}.
...
automorphisms of a knot shadow, which rapidly decreases exponentially to 1 (the
identity map), as is known for sufficiently large numbers of
crossings~\cite{Chapman2015knotasymp}.
...
appearing exactly once. It is not clear to us why this phenomenon should occur
in the data: this data is certainly compatible with the hypothesis that there is
an (asymptotic) power-law relationship between knot rank and knot probability
akin to Zipf's law (cf.~\cite{Orlandini:2007PRL}), but it would take much larger experiments to provide strong statistical support for such a conjecture.
...
We also observe that two pairs of vertices $ab$ and $cd$ on the unit circle may be joined by nonintersecting chords inside the circle if and only if the pairs are unlinked on the circle (\raisebox{-0.032in}{\includegraphics[width=0.15in]{unlinked-on-circle.pdf}}
instead of \raisebox{-0.032in}{\includegraphics[width=0.15in]{linked-on-circle.pdf}}). This happens when the order of the vertices around the circle is $abcd$ instead of $acbd$ or $adbc$.
