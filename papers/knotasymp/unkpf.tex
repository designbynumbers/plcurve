...
may differ by one from the number of link components in the
...
\begin{figure}[h!]
  ...
  \begin{scope}[every path/.style={very thick}, every node/.style={circle,fill=black, inner sep=2pt}]
    ...
    \draw (T) -- (Z) {};
    \draw (T) -- (Y) -- (O) -- (X) -- (T) {};
    \draw (Y) .. controls (2.1, 1.7) and (1.2, 2) .. (T);
    \draw (O) -- (B);
    \draw (T) .. controls (-3,2) and (-1.5,-.5) .. (B);
    \draw (T) .. controls (3,2) and (1.5,-.5) .. (B);
  \end{scope}
  \end{tikzpicture}
...
condition is exactly the pattern theorem for 2-tangles proved in the
prior section!

For reduced diagrams, we must pick a slightly different $M_1$, such as
the one in figure ???
