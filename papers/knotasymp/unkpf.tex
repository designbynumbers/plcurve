\documentclass[amsmath,longbibliography,secnumarabic,floatfix,amssymb,nofootinbib,nobibnotes,letterpaper,11pt,tightenlines,notitlepage,showkeys,showlabels]{revtex4-1}
...
lattice walks [cite], random space polygons [cite], random braid words
[cite], \emph{Petaluma} [cite], et.\ al. In this paper we will discuss
the \emph{random diagram model} under which \emph{knot diagrams} are
drawn uniformly from the set of all diagrams with a given number of
crossings. Alternating knot and link diagrams have been studied [cite]
but little is published about the knottiness of arbitrary random
diagrams of large size.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[every path/.style={string, very thick}, every node/.style={transform shape,
    ...
    \begin{scope}
      ...
      \draw (A) -- (X.center);
      \draw (X.center) .. controls (X.16 north east) and (X.16 north west) .. (X);
      \draw (X) -- (B);
    \end{scope}
...
A \emph{$2k$-leg curve} is a generic immersion of $k$ intervals into
the sphere so that all $2k$ interval ends lie in the same face of
the sphere. A \emph{$2k$-tangle} is a $2k$-leg curve together with
over-under information at each double point.
\begin{figure}[h!]
  \centering
  \caption{A $6$-leg curve, and a $6$-tangle with its underlying structure.}
  \label{fig:2klegcurve}
\end{figure}

\subsubsection{Combinatorial maps}
\label{sec:combmapdefs}

We wish to examine the underlying \emph{map} structure of knot and
link diagrams. If $\ArbSurf$ is a surface, then its Euler
characteristic $\chi(\ArbSurf)$ is a topological invariant. The type
$g$ of a surface is defined by $\chi(\ArbSurf) = 2 - 2g$ (this
definition agrees with the genus $g$ of orientable surfaces).

\begin{definition}
  A \emph{map with $n$ vertices} $M$ is a graph $\GraphOf M$ embedded on
  a surface $\ArbSurf$ of type $g$ so that every connected component
  of $\ArbSurf \setminus M$ is a topological disk. The connected
  components of $\ArbSurf \setminus M$ are called the \emph{faces} of
  $M$. The map $M$ is \emph{4-regular} or \emph{quartic} if every
  vertex in the underlying graph $\GraphOf M$ has degree 4. A
  \emph{rooted map} is a map together with a single edge marked with a
  direction.

  If $\ArbSurf$ is the sphere, then the map $M$ is called \emph{planar}.
\end{definition}

In this paper we will only consider $\ArbSurf$ to be an orientable
surface (of genus $g$). Considering $g \ne 0$ extends results to
\emph{virtual} diagrams, but we will be most concerned with results
for classical knot theory, where $\ArbSurf = S^2$.
...
\end{figure}
There are alternate, equivalent definitions of rooting a map. Let $M$
be a rooted map with root edge $\rho$ which points from vertices $v_1$
to $v_2$. Then the unique face $f$ which $\rho$ goes counterclockwise
around is the \emph{root face}. The root face is the \emph{exterior
  face}, and is typically drawn as such. The vertex $v_2$ at the tip
of $\rho$ is the \emph{root vertex}.

Maps have a well defined notion of the \emph{dual map}; a map $M = (V,
...
A map $M$ is a CW-complex for the surface in which it embeds. Given a
cycle $\gamma$ of edges in $M$,

\begin{definition}
  A rooted map $P$ is a \emph{submap} of a larger (possibly unrooted)
  map $M$ if there exists a cycle of $k$ (possibly repeated) edges in
  $M$ so that one of the two halves of $M$ separated by the cycle is
  identical
\end{definition}
...
\begin{definition} A \emph{(rooted) knot diagram} is a (rooted) link diagram which consists of only
  ...
  $\KnotShad_n$.
\end{definition} Knot shadows $\KnotShad_n$ represent a curious, small
subclass of $\LinkShad_n$.

To prove asymptotics for shadows, diagrams, and arbitrary other
flavors, we introduce the idea of a labeled map;
\begin{definition}
  Given a triple of label sets $S = (S_V, S_E, S_F)$, an
  \emph{$S$-map} or \emph{$S$-labeled map} is a map $M = (V,E,F)$ together with a triple of maps
  $(s_V, s_E, s_F)$, $s_*: * \to S_*$ which label each vertex, edge,
  and face of $M$ with an element of $S$.
\end{definition}
Unless mentioned otherwise, we will only consider vertex-labeled maps,
and so abuse notation with $S := S(S, \{0\}, \{0\})$. Call a link
(resp. knot) shadow labeled with a set $S$ as link (knot)
$S$-shadow. We will show in corollary~\ref{cor:linkdiaaresshadows} that link
(resp. knot) diagrams are exactly link (knot) $\{+, -\}$-shadows. With
this in mind we can define other useful classes of $S$-shadows, like
pseudodiagrams:
\begin{definition}
  A link (resp. knot) pseudodiagram is a link (knot) shadow, together with
  over-under information at some subset of vertices in the
  shadow. Equivalently (c.f.\ corollary \ref{cor:linkdiaaresshadows}), a
  pseudodiagram is a $\{+, 0, -\}$-shadow.
\end{definition}

\begin{definition}
  A link \emph{HOMFLY skein resolution diagram} is a link shadow where each
  vertex is either of crossing type (and has over-under information),
  or of smooth type, where it represents the valid skein tangency
  which keeps edge orientations consistent. Equivalently, it is a
  $\{+,-,s\}$-shadow.
\end{definition}
The number of link components in a HOMFLY skein resolution diagram
may differ from the number of link components in the
shadow. Hence the class of knot HOMFLY skein resolution diagrams is
\emph{different} from the class of knot pseudodiagrams, despite having
the same labeling set.

TODO: I'd like to be able to say here (or later) that we can include
labels which correspond to tangencies (rather than crossings!). I
argue that from a consistent orientation on the edges of the shadow,
labels which represent either skein a or skein b can be consistently
parsed.
