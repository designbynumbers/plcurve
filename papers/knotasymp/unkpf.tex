...
Is a canonical way to order crossings, components, edges, and direct edges consistently for a rooted
link shadow:
\begin{enumerate}
\item Root vertex is vertex 1, root edge is edge 1.
\item Empty stack of components to label; add root component.
\item Pop component $C$ from stack, labelling vertices and edges in order.
\item Add components to stack; orient the positive edge at first appearance of component away from
  crossing.
\item Repeat until stack empty.
\end{enumerate}
...
\item \textbf{Connect sum.} Let $L$ be dual to a rooted link shadow, and $Q$ be the dual to a rooted
  ...
  \end{figure}
\item \textbf{4-tangle replacement.} Let $L$ and $Q$ be duals to
  rooted link shadows. Canonically index and direct the edges of $L$
  via the blossom tree bijection. Given a face $f$ in $L$ with
  lowest-index edge $e$, define the 4-tangle replacement as
  follows. Delete $f$ from $L$ and the root face of $Q$ to create two
  quadrangulations $L'$, $Q'$ with quadrangle boundary. Then glue $Q'$
  to $L'$ along their boundary faces so that the root of $Q'$ is glued
  to $e$ with directions matching.
\end{enumerate}
...
$\ArbSubClass$ some other class with generating function $\sum_{n=0}^{\infty}{\arbsubclass_n~z^n}$
to be specified later ($\ArbSubClass$ will be some sufficiently nice class of objects indecomposable
under a composition operation we will define). Suppose we have some preliminary, loose bound away
from zero on $r$ (there are at most $12^n$ planar maps, and so in our cases we have $r \ge 1/12$)
and that $r \le 1$ Let $C_i > 0$ and $1-r>\delta>0$ be arbitrary. The method involves three steps;
...
\begin{proof}
  A very loose bound on $r = r(K)$ is $1/12 \le r \le 1$ as the number of planar maps in
  general is bounded by $12^n$. Let $C_i > 0$ and $1-r > \delta > 0$
  be arbitrary.

  \begin{enumerate}
  \item \emph{Show that there exists some $n \ge 0$ with $k_n > C_1(r
      + \delta)^{-n}$.} This follows immediately from the
    Cauchy-Hadamard theorem.
  \item \emph{Show that there is a subclass $\ArbSubClass$ of $\KnotShad$ for which there exists
      some $m \ge 0$ with $\arbsubclass_m > C_2(r + \delta)^{-m}$ and $\arbsubclass_{m+1} > C_2(r +
      \delta)^{-(m+1)}$.} Recall that we want to pick $\ArbSubClass$ so that its objects are
    \emph{indecomposable} under a composition operation $+$. Hence to motivate our upcoming choice
    of $\ArbSubClass$, we will first define $+$: Define the concatenation $K = K_1+K_2$ on
    shadows by gluing the front leg of $K_1$ to the hind leg of $K_2$.
    \begin{figure}[h!]  \centering
      \[
      \begin{aligned}
    ...
    \end{figure}
    Hence, we will take $\ArbSubClass$ to be the class of knot shadows which remain connected after
    removing the root edge and any additional non-root edge.
    ...
    \end{figure} Then as $d_n$ counts knot shadows which are at least 2-connected as 2-leg diagrams,
    and since $\varphi^2$ acts similarly while increasing the number of crossings by 2, we obtain
    step 2 as by the discussion in the prior section.
  \item \emph{Show that there exists some $N$ so that for any $n \ge N$, $k_n > (r + \delta)^{-n}$.}
    We only need show that $+$ satisfies the appropriate properties laid out in the previous
    section. Certainly $A + B \in \KnotShad$ as we obtain a new 2-leg shadow. As $+$ introduces no
    crossings, we have $k = 0$ and $|A + B| = |A| + |B|$. Finally, a 2-leg shadow $K$ either lies in
    $\ArbSubClass$ or has $\ell-1$ disconnecting edges. Cutting these edges produces the disjoint
    union $\ell$ well-ordered 2-leg shadows (well ordered from their position in the long curve $K$)
